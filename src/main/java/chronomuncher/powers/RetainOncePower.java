...
public class RetainOncePower extends AbstractPower
{
	...
	{
		if (this.owner.hasPower(RetainCardPower.POWER_ID)) {
			this.owner.getPower(RetainCardPower.POWER_ID).amount -= this.permaRetain;
		}
	...
	public void atEndOfTurn(boolean isPlayer)
	{
		if (this.owner.hasPower(RetainCardPower.POWER_ID)) {
			...
			this.owner.getPower(RetainCardPower.POWER_ID).amount += this.amount;
			this.amount = 0;
		}
		else if (!AbstractDungeon.player.hand.isEmpty()
				&& !this.owner.hasPower(EquilibriumPower.POWER_ID)
